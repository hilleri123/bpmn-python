...
class BpmnDiagramGraphImport(object):
    ...
    def load_diagram_from_xml(filepath, bpmn_diagram):
        ...
        diagram_element = document.getElementsByTagNameNS(
            "*", "BPMNDiagram")[0]
        plane_element = diagram_element.getElementsByTagNameNS(
            "*", "BPMNPlane")[0]
        ...
        if collaboration_element_list is not None and len(collaboration_element_list) > 0:
            ...
            BpmnDiagramGraphImport.import_collaboration_element(
                diagram_graph, collaboration_element, collaboration)
        ...
        for element in utils.BpmnImportUtils.iterate_elements(plane_element):
            if element.nodeType != element.TEXT_NODE:
                ...
                if tag_name == consts.Consts.bpmn_shape:
                    BpmnDiagramGraphImport.import_shape_di(
                        participants, diagram_graph, element)
                elif tag_name == consts.Consts.bpmn_edge:
                    BpmnDiagramGraphImport.import_flow_di(
                        diagram_graph, sequence_flows, message_flows, element)
    ...
    def import_collaboration_element(diagram_graph, collaboration_element, collaboration_dict):
        ...
        """
        collaboration_dict[consts.Consts.id] = collaboration_element.getAttribute(
            consts.Consts.id)
            ...
            if element.nodeType != element.TEXT_NODE:
                ...
                if tag_name == consts.Consts.participant:
                    BpmnDiagramGraphImport.import_participant_element(
                        diagram_graph, participants_dict, element)
                elif tag_name == consts.Consts.message_flow:
                    BpmnDiagramGraphImport.import_message_flow_to_graph(
                        diagram_graph, message_flows_dict, element)
    ...
    def import_participant_element(diagram_graph, participants_dictionary, participant_element):
        ...
        process_ref = participant_element.getAttribute(
            consts.Consts.process_ref)
        ...
        participants_dictionary[participant_id] = {
            consts.Consts.name: name, consts.Consts.process_ref: process_ref}
    ...
    def import_diagram_and_plane_attributes(diagram_attributes, plane_attributes, diagram_element, plane_element):
        ...
        """
        diagram_attributes[consts.Consts.id] = diagram_element.getAttribute(
            consts.Consts.id)
        ...
        plane_attributes[consts.Consts.id] = plane_element.getAttribute(
            consts.Consts.id)
    ...
    def import_process_elements(document, diagram_graph, sequence_flows, process_elements_dict, plane_element):
        ...
        for process_element in document.getElementsByTagNameNS("*", consts.Consts.process):
            BpmnDiagramGraphImport.import_process_element(
                process_elements_dict, process_element)

            process_id = process_element.getAttribute(consts.Consts.id)
            process_attributes = process_elements_dict[process_id]

            lane_set_list = process_element.getElementsByTagNameNS(
                "*", consts.Consts.lane_set)
            if lane_set_list is not None and len(lane_set_list) > 0:
                ...
                BpmnDiagramGraphImport.import_lane_set_element(
                    process_attributes, lane_set, plane_element)
    ...
    def __import_element_by_tag_name(diagram_graph, sequence_flows, process_id, process_attributes, element, tag_name):
        if tag_name == consts.Consts.task \
            ...
            BpmnDiagramGraphImport.import_task_to_graph(
                diagram_graph, process_id, process_attributes, element)
            ...
            BpmnDiagramGraphImport.import_start_event_to_graph(
                diagram_graph, process_id, process_attributes, element)
        elif tag_name == consts.Consts.end_event:
            BpmnDiagramGraphImport.import_end_event_to_graph(
                diagram_graph, process_id, process_attributes, element)
        ...
        for element in utils.BpmnImportUtils.iterate_elements(lane_set_element):
            if element.nodeType != element.TEXT_NODE:
                ...
                if tag_name == consts.Consts.lane:
                    ...
                    lane_attr = BpmnDiagramGraphImport.import_lane_element(
                        lane, plane_element)
                    lanes_attr[lane_id] = lane_attr

        lane_set_attr = {consts.Consts.id: lane_set_id,
                         consts.Consts.lanes: lanes_attr}
    ...
    def import_child_lane_set_element(child_lane_set_element, plane_element):
        ...
        child_lane_set_attr = {
            consts.Consts.id: lane_set_id, consts.Consts.lanes: lanes_attr}
    ...
    def import_lane_element(lane_element, plane_element):
        ...
        if shape_element is not None:
            ...
            lane_attr[consts.Consts.width] = bounds.getAttribute(
                consts.Consts.width)
            lane_attr[consts.Consts.height] = bounds.getAttribute(
    ...
    def import_process_element(process_elements_dict, process_element):
        ...
        process_element_attributes = {consts.Consts.id: process_element.getAttribute(consts.Consts.id),
                                      consts.Consts.name: process_element.getAttribute(
                                          consts.Consts.name)
                                      if process_element.hasAttribute(consts.Consts.name) else "",
                                      consts.Consts.is_closed: process_element.getAttribute(
    ...
    def import_flow_node_to_graph(bpmn_graph, process_id, process_attributes, flow_node_element):
        ...
        bpmn_graph.nodes[element_id][consts.Consts.type] = \
            utils.BpmnImportUtils.remove_namespace_from_tag_name(
                flow_node_element.tagName)
    ...
    def import_task_to_graph(diagram_graph, process_id, process_attributes, task_element):
        ...
        """
        BpmnDiagramGraphImport.import_activity_to_graph(
            diagram_graph, process_id, process_attributes, task_element)
    ...
    def import_activity_to_graph(diagram_graph, process_id, process_attributes, element):
        ...
        """
        BpmnDiagramGraphImport.import_flow_node_to_graph(
            diagram_graph, process_id, process_attributes, element)
    ...
    def import_complex_gateway_to_graph(diagram_graph, process_id, process_attributes, element):
        ...
        """
        element_id = element.getAttribute(consts.Consts.id)
        BpmnDiagramGraphImport.import_gateway_to_graph(
            diagram_graph, process_id, process_attributes, element)
        diagram_graph.nodes[element_id][consts.Consts.default] = element.getAttribute(consts.Consts.default) \
            if element.hasAttribute(consts.Consts.default) else None
    ...
    def import_event_definition_elements(diagram_graph, element, event_definitions):
        ...
        for definition_type in event_definitions:
            event_def_xml = element.getElementsByTagNameNS(
                "*", definition_type)
    ...
    def import_start_event_to_graph(diagram_graph, process_id, process_attributes, element):
        ...
        BpmnDiagramGraphImport.import_event_definition_elements(
    ...
    def import_sequence_flow_to_graph(diagram_graph, sequence_flows, process_id, flow_element):
        ...
        name = flow_element.getAttribute(
            consts.Consts.name) if flow_element.hasAttribute(consts.Consts.name) else ""
    ...
    def import_shape_di(participants_dict, diagram_graph, shape_element):
        ...
        if diagram_graph.has_node(element_id):
            ...
            node[consts.Consts.width] = bounds.getAttribute(
                consts.Consts.width)
            node[consts.Consts.height] = bounds.getAttribute(
                consts.Consts.height)
        ...
        if element_id in participants_dict:
            ...
            participant_attr[consts.Consts.is_horizontal] = shape_element.getAttribute(
                consts.Consts.is_horizontal)
            participant_attr[consts.Consts.width] = bounds.getAttribute(
                consts.Consts.width)
            participant_attr[consts.Consts.height] = bounds.getAttribute(
                consts.Consts.height)
            participant_attr[consts.Consts.x] = bounds.getAttribute(
                consts.Consts.x)
            participant_attr[consts.Consts.y] = bounds.getAttribute(
                consts.Consts.y)
    ...
    def import_flow_di(diagram_graph, sequence_flows, message_flows, flow_element):
        ...
        waypoints_xml = flow_element.getElementsByTagNameNS(
            "*", consts.Consts.waypoint)
